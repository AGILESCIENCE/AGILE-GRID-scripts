...
tmplogdir=$HOME/tmplog_agileobs
tmpevtdir=$HOME/tmpevt_agileobs
tmpevtdirft=$HOME/tmpevtft_agileobs
tmpauxdir=$HOME/tmpaux_agileobs
tmpcordir=$HOME/tmpcor_agileobs
...
aux_files=$(mysql -s -B -hagiles9 -ugs agile2 -e "SELECT CONCAT('/storage1/agile/agile2/aux/data/',path,'/',filename) FROM Auxiliary WHERE TYPE in ('EARTH', 'SAS', 'ACSMAN', 'SOE', 'OPM', 'PMS', 'TCSP', 'OBTUTC') AND ArrivalDate>=DATE_SUB(NOW(), INTERVAL 1 YEAR);")
