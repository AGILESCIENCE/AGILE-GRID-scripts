...
aux_files=$(mysql -s -B -hagiles9 -ugs agile2 -e "SELECT CONCAT('/storage1/agile/agile2/aux/data/',path,'/',filename) FROM Auxiliary WHERE TYPE in ('EARTH', 'SAS', 'ACSMAN', 'SOE', 'OPM', 'PMS', 'TCSP', 'TCO') AND ArrivalDate>=DATE_SUB(NOW(), INTERVAL 1 YEAR);")
for file in $aux_files ; do
    ln -s $file $tmpauxdir/$(basename $file)
done
rm -r $tmpcordir &> /dev/null
mkdir $tmpcordir
cor_files=$(mysql -s -B -hagiles9 -ugs agile2 -e "SELECT CONCAT('/storage1/agile/agile2/LV1corr','/',filename) FROM IndexL1Corr WHERE CHANNEL=1 AND datemin>=\"$datemin\" AND datemin<=now();")
