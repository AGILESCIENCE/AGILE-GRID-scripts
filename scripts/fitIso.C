...
void fitIso(string filename, int experiment, TCanvas* c1, int overlap, int color=kBlue) {
	...
	for(int i=1; i<=n; i++) {
   	...
   	c1->cd(1);
   	if(overlap == 1) {
   		gr->Draw("AL"); //also AL for histos or LP* for graph
   		gr->SetLineColor(color);
   	} else {
   		gr->Draw("L"); //also L for histo or ALP* for graph
   		gr->SetLineColor(color);
   	}
	////////////////////////////////////////////////////////////////////////////////
   	//Prefactor = $N_0$ par[0]
...
*/
   ...
   // gr->Fit(fitfun);
	...
	for(int i=1; i<=n; i++)
		//gr->SetBinContent(i, y[i-1]);
		cout << "value [" << edges[i-1] << ", " << edges[i] << "] " << ": " << (*fitfun)(edges[i]-edges[i-1]) << endl;
