...
for i in ${!expfiles[*]} ; do
    ...
    AG_add_diff diffusefilelist=${convfilelist} edpfile=${caldir}/AG_GRID_G0017_SFMG_$5.edp.gz sarfile=${caldir}/AG_GRID_G0017_SFMG_$5.sar.gz outfile=\!${dispfile} emin=${addemin} emax=${addemax} > ${dispfile}.out 2>&1
