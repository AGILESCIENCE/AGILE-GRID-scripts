...
void repeatedPostTrialLD(int N = 180, int K = 10, int hp = 1, double TS = 25, double delta=1, int M = 1, int N1 = 1, double eta1=0.45, int N2 = 2, double eta2=4.8e-03 ) {
	...
	LongDouble_t post = 1.0 - TMath::Power(1. - pre, (Long64_t) N);
	if(post == 0) {
	...
	cout << "p-value post-trial = 1 - (1 - p)^N = " << (long double) post << endl;
	...
	*/
	...
	sumA = 0.0;
	for(Long64_t j=K-1; j>=0; j--) {
		LongDouble_t pr2 = ( TMath::Binomial(N, j) * TMath::Power(pre, (Long64_t) j) * TMath::Power(1.0-pre, (Long64_t) N-j) );
		sumA  += pr2;
		//printf("%i      %1.60Lf	%1.60Lf\n", j, sumA, pr2);
	}
	//cout << sumA << endl;
	LongDouble_t resultsumA = 1. - TMath::Power(sumA, (Long64_t) M);
	//printf("resultsumA     %4.30Lf\n", resultsumA);
	
	LongDouble_t sum=0.;
	for(Long64_t j=K-1; j>=0; j--){
		LongDouble_t bin = TMath::Binomial(N, j);
		LongDouble_t p1 = TMath::Power(pre, j);
		//printf("p1      %1.30Lf\n", p1);
		//long double p2 = (1. - pre)**(N-j);
		LongDouble_t p2 = TMath::Power(1.-pre, N-j);
		//printf("p2      %1.30Lf\n", p2);
		sum = sum + (bin * p1 * p2);
		//printf("A%2i %1.50Lf\n", j, sum);
		//printf("B%2i %1.50Lf\n", j, pippo);
	...
	LongDouble_t resultsum = 1. - TMath::Power(sum, (Long64_t)1);
	...
	if(hp == 2) cout << "r = 3 (\delta = " << delta << ")" << endl;
	//cout << "p = " << pre << endl;
	cout << "p = " << pre << endl << endl;
	//cout << "Pretrial significance" << endl;
	//printf("1) p-value sum      %6.16e\n", resultsum);
	cout << "P(N,X≥k)=1-\sum_{j=0}^{k-1} [ (N j) p^j (1-p)^{N-j} ] = " << (long double)resultsum << endl;
	cout << "It correspond to " <<	(long double) TMath::ErfInverse(1.0-resultsum)*TMath::Sqrt(2) << " (normal) sigma \n" << endl;
	...
	LongDouble_t result = 1.0 - TMath::Power(1.0 - (LongDouble_t)TMath::BinomialI(pre , N, K), (Long64_t)M);
	cout << "\nMethod 1:"<<endl;
	...
	LongDouble_t result2 = 1.0 - TMath::Power(sum, (Long64_t)M);
	cout << "Method 2:"<<endl;
	printf("P(N,X≥k)=1-( \sum_{j=0}^{k-1} [ (N j) p^j (1-p)^{N-j} ] )^M = %.6e\n", result2);
