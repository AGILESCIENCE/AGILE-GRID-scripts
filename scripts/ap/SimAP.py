...
class SimAP:
    ...
    def calculateAPLC(self, help=1, apfile='', ranal= 1, gasvalue=0.0006, gal = 0.7, iso = 10., emin = 100., emax = 10000., period = 729855.36, expdatafixed=0, peak_size = 50e-08, instrumentID=0):
        if (help == "1"):
            ...
            print('mean rate true: %.3e +/- %.3e ' % (meanratetrue/len(tstartA), math.sqrt(meanratetrueerr) / expsum))

            print('mean rate sim : %.3e +/- %.3e ' % (meanratesim/len(tstartA), math.sqrt(meanratesimerr) / expsum))
