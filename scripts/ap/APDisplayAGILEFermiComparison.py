...
class APDisplayAGILEFermiComparison:
    ...
    def plot(self, ax, agile_data, fermi_data, arg_lines, plotrate):
        ...
        tm = (self.time_tt_to_mjd(agile_data["tstart"]) + self.time_tt_to_mjd(agile_data["tstop"])) / 2
        ...
        if plotrate:
            ...
            tw = tm -  self.time_tt_to_mjd(agile_data["tstart"])
        ...
        tmFermi = (self.time_tt_to_mjd(fermi_data["tstart"]) + self.time_tt_to_mjd(fermi_data["tstop"])) / 2
    ...
    def checkSignificance(self, fermi, tstart, tstop):
        ...
        for time in range(int(tstart), int(tstop)):
            #print(time)
            tstart_tt = self.time_mjd_to_tt(time)
            tstop_tt = self.time_mjd_to_tt(time+1)
