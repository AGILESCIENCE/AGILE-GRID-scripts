...
class GammaAP:
    ...
    def calculateLS():
        ...
        if scyp == 1:
            nout = 100000
            f = np.linspace(10e-9, 10e-4, nout)
            pgram = signal.lombscargle(self.tstartA, self.ctsdataA, f)
            plt.subplot(2, 1, 1)
            plt.plot(tstartA, expdataA)
            plt.subplot(2, 1, 2)
