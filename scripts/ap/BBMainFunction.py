"""
Created on Mon Mar  9 11:55:10 2020
...
def main():
    ...
    if a[len(a)-4:len(a)] == '.ap3':
        ...
        print('Press 1 to create the Bayesian blocks Dataset \n'
              'Press 2 to see All Dataset results \n'
              'Press 3 to see Real Time results \n'
              'Press q to quit')
        i = input()
        if i == '1':
            BBdataset(m)
...
#                    N_s[i] = 0
        ...
        work1 = t_tsorted
        h = np.histogram(work1, range=[0,np.int(max(work1))+1], bins = np.int(max(work1))+1)
        N_tot = h[0]
...
def BBdataset(data):
    ...
    if esponente[-3:] == '-01':
        x = np.round(data[:,1]*1e+02) # frequenza eventi per ogni intervallo di tempo
    else:
        x = np.round(data[:,1]*1e+07) # frequenza eventi per ogni intervallo di tempo
    ...
    c = 0 #counts
    (b2,b3,mean_block,err_block,lista1,lista2) = (list(),list(),list(),list(),list(),list()) 
    ...
    m = pd.DataFrame(m)
    m = m.rename(columns={0: "tstart", 1: "tstop", 2: "FluxMeanBB", 3: "FluxErrorBB"})
    ...
    d = pd.DataFrame(d)
    d = d.rename(columns={0: "tstart", 1: "tstop", 2: "FluxMeanBB", 3: "FluxErrorBB"})
    d.to_csv('BBdataset2.csv',index = True, sep = ';')
    return(m)
            
def AllData(data,N_b,N_s):
   ...
   #Function of Bayesian block
    ...
    for i in range(len(b2)):
        S_block.append((np.sqrt(2)*np.sqrt((np.sum(b3[i])+np.sum(b4[i]))*np.log(2*(np.sum(b3[i])+np.sum(b4[i]))/(np.sum(b3[i])+2*np.sum(b4[i])))+np.sum(b4[i])*np.log(2*np.sum(b4[i])/(np.sum(b3[i])+2*np.sum(b4[i])))))/sum(b5[i]))
    ...
    # Data
    plt.plot(data[:,0], data[:,1], 'b_', markersize = 3)
    # Error of data (col.20)
    plt.plot((data[:,0],data[:,0]), (data[:,1]-data[:,2],data[:,1]+data[:,2]), 'b', linewidth=.5, alpha = 0.5)
    ...
    plt.plot((edges[len(edges)-1]+21600/2,edges[len(edges)-1]+21600/2),(SNR_block[len(SNR_block)-1],0), 'g', linewidth=1)
    # Threshold
    plt.plot(data[:,0],(np.mean(N_s)/np.mean(N_s+2*N_b))/np.mean(data[:,3])*5*np.ones(117), 'r:', linewidth = 1)
    ...
    plt.plot((edges[len(edges)-1]+21600/2,edges[len(edges)-1]+21600/2),(S_block[len(S_block)-1],0), 'g', linewidth=1)
    # Threshold
    plt.plot(data[:,0],(np.mean(N_s)/np.mean(N_s+2*N_b))/np.mean(data[:,3])*5*np.ones(117), 'r:', linewidth = 1)
    # Condition that gives us which values are over the  threshold = 3
    count = 0
    ...
    plt.title('Significance (S)')
    plt.xlabel('TT time *10$^8$')
    plt.ylabel('S')
    print(count)
    plt.show()

def RealTime(data,N_b,N_s):
    # SNR is a measure that compares the level of a desired signal to the level of background noise.
    ...
    for i in range(len(data[:,0])):
        ...
        if i > 1:
            ...
            edges = bayesian_blocks(t, x, fitness='events', gamma = 3*1e-07) #bayesian blocks function
            ...
            for j in range(len(a2_cum)):
                b2.append(data[c:a2_cum[j],1]) # data[left_edge_block:right_edge_block] 
                b6.append(data[c:a2_cum[j],2])
                mean_block.append(np.mean(b2[j]))
        ...
        if i > 1:
            ...
            for j in range(len(a2_cum)):
                ...
                b3.append(N_s[c:a2_cum[j]])
                b5.append(data[c:a2_cum[j],3])
                SNR_block.append((np.sum(b3[j])/np.sqrt(np.sum(b3[j])+2*np.sum(b4[j])))/sum(b5[j]))
                c = a2_cum[j]
            if len(SNR_block)>1:
                # Plot of SNR
                plt.plot(((edges[1:]+edges[:-1])/2-(edges[1:]-edges[:-1])/2,(edges[1:]+edges[:-1])/2+(edges[1:]-edges[:-1])/2),(SNR_block,SNR_block), 'g', linewidth=1)
                ...
                # Threshold
                plt.plot(data[:i+1,0],(np.mean(N_s[:i+1])/np.mean(N_s[:i+1]+2*N_b[:i+1]))/np.mean(data[:i+1,3])*5*np.ones(len(data[:i+1,0])), 'r:', linewidth = 1)
                # Condition that gives us which values are over the  threshold = 3
            ...
            for j in range(len(a2_cum)):
                ...
                b3.append(N_s[c:a2_cum[j]])
                b5.append(data[c:a2_cum[j],3])
                S_block.append(np.sqrt(2)*np.sqrt((np.sum(b3[j])+np.sum(b4[j]))*np.log(2*(np.sum(b3[j])+np.sum(b4[j]))/(np.sum(b3[j])+2*np.sum(b4[j])))+np.sum(b4[j])*np.log(2*np.sum(b4[j])/(np.sum(b3[j])+2*np.sum(b4[j]))))/sum(b5[j]))
                c = a2_cum[j]
            if len(S_block)>1:
                # Value of S
                plt.plot(((edges[1:]+edges[:-1])/2-(edges[1:]-edges[:-1])/2,(edges[1:]+edges[:-1])/2+(edges[1:]-edges[:-1])/2),(S_block,S_block), 'g', linewidth=1)
